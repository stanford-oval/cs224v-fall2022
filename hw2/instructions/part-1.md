...
- [Data synthesis](#prepare-training-data-for-a-domain-of-your-choice)
- [Train a semantic parser](#train-a-semantic-parser)
- [Evaluate the semantic parser](#evaluate-the-semantic-parser)
- [See model outputs in ThingTalk](#see-model-outputs-in-thingtalk)
...
## See model outputs in ThingTalk
...
Manually create a text file which contains your queries line by line. For example, consider `queries.txt` with the following content:
```
list a city
list any one city
list a city near rome?
which administrative territories share border with rome?
which administrative territories are the sister cities of versailles?
which states are vienna a part of?
```

Run the following command to obtain the ThingTalk output generated by the model:
```
python get_output.py --query_file <your_text_file_with_queries>
```
...
- At least five commands you've tried along with their ThingTalk outputs
